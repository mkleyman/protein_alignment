...
public class SplineWriter {
    public static void writeFile(String filename,  TreeBasedTable<String,Double,Double> human_table,
        ...
        line = "mouse,human,type,time,expression,correlation\n";
        pw.print(line);
        ...
        for(String mouseProtein:homologDict.keySet()){
            double correlation = aligner.align_polynomial_single(splineDictionary,funct,mouseProtein);
            humanProtein = homologDict.get(mouseProtein);
            for(Double time: mouse_table.columnKeySet()){
                time_transformed = funct.value(time);
                line = mouseProtein+","+humanProtein+",mouse,"+time+","+mouse_table.get(mouseProtein,time)+","+correlation+"\n";
                pw.print(line);
            ...
            for(Double time: human_table.columnKeySet()){
                line = mouseProtein+","+humanProtein+",human,"+time+","+human_table.get(humanProtein,time)+","+correlation
                        +"\n";
                pw.print(line);
            }
            ...
            for(double time: sTimes){
                line = mouseProtein+","+humanProtein+",mouse_spline,"+time+","+
                        splineDictionary.getSpline(mouseProtein).evaluate(time)+","+correlation+"\n";
                pw.print(line);
                time_transformed = funct.value(time);
                try {
                    ...
                    line = mouseProtein + "," + humanProtein + ",human_spline," + time_transformed + "," +
                            splineDictionary.getSpline(humanProtein).evaluate(time_transformed) +","+correlation+ "\n";
                    pw.print(line);
                }catch(Exception e){}
